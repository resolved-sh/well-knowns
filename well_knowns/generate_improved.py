...
def is_valid_record(record: dict) -> bool:
    ...
    for path, ep in record["endpoints"].items():
        if ep.get("status") == 200 and path.endswith(".json"):
            # Require actual data, not just HTTP 200
            if ep.get("data") is None:
                return False
            # Reject error responses wrapped in HTTP 200
            if isinstance(ep.get("data"), dict) and ep["data"].get("status") and ep["data"]["status"] >= 400:
                return False
    return True
