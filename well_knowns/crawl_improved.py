...
CONCURRENT_LIMIT  = 800    # Increased for faster crawling
REQUEST_TIMEOUT   = 2.0     # Slightly increased timeout for reliability
BATCH_SIZE        = 5_000   # Increased checkpoint interval to reduce I/O overhead
...
async def probe_domain(client: httpx.AsyncClient, domain: str, rank: int = None) -> dict:
    ...
    async def probe_one(path: str) -> tuple[str, dict]:
        ...
        try:
            r = await client.get(url, timeout=REQUEST_TIMEOUT, follow_redirects=False)  # Do NOT follow redirects
            ...
            if r.status_code == 200:
                ...
                ep["error"] = f"redirect_{r.status_code}"  # Treat as not implemented, don't follow
...
async def run_batch(domains: list[str], output_path: Path, resume: bool = True):
    ...
    async with httpx.AsyncClient(
        ...
        for i, domain in enumerate(domains):
            ...
            if i > 0 and i % 1000 == 0:
                await asyncio.sleep(0.5)  # Brief pause every 1000 domains
